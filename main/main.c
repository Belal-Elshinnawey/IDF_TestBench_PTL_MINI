...
#define LEDC_DUTY (84)                // Set duty to 50%. ((2 ** 13) - 1) * 25% = 2048
...
int32_t FIRFilter_calc(FIRFilter *fir, int32_t inputVal){
    ...
    for(int i=0;i<FILTER_TAP_NUM;i++){
        ...
        acc = acc +fir->buff[sumIndex]*filter_taps[i];
    }
    fir->out= acc;
...
void app_main(void)
{
    ...
    while(1) {
        ...
        if (ret == ESP_OK || ret == ESP_ERR_INVALID_STATE) {
            ...
            printf("lowerlimit:0,Channel0:%d,Channel3:%d,Channel4:%d,Channel5:%d,Channel6:%d,Channel7:%d,upperlimit:1.6e8\n",out0, out3, out6, out5, out4, out7);
